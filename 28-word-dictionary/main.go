...
func wordLookup(word string) string{
	...
	file, err := os.Open(csvFilePath)
	logError("Error opening the index file", err)
...
func merge(){
	...
	for dictWord != "" || chglogWord != "" {
		...
		}else if dictWord == "" || strings.Compare(chglogWord, dictWord) <= 0{ // = in case the word exist in both
			newDict.WriteString(fmt.Sprintf("%s,%s\n", chglogWord, chglogMeaning))
			newIndex.WriteString(fmt.Sprintf("%s %d\n", chglogWord, offset))
			offset += int64(len(chglogWord) + len(chglogMeaning) + 2) + int64(len("\r\n"))
...
func addWordToChangelog(word, meaning string){
	...
	fmt.Sprintf("Word added to changelog: %s\n", word)
}
...
func main(){
	...
	addWord("Funnny", "A trait many people lack.")
